...
import { IfcModelWorkspace } from '../features/ifc-viewer/IfcModelWorkspace'
...
export function ProjectEditorPage() {
  ...
  const totalUnits = project?.current_version?.program.mix.reduce(
    (total, entry) => total + entry.count,
    0,
  ) ?? 0

  useEffect(() => {
    if (activeTab === 'model' && !hasIfcOutput) {
      setActiveTab('program')
    }
  }, [activeTab, hasIfcOutput])
    ...
    <div className="space-y-6">
      <section className="panel-surface rounded-[2rem] p-6 md:p-8">
        <PageHeader
          ...
          title={project.name}
          description={project.description || t('project_editor.description_fallback')}
          actions={
            <Link
              to="/projekte"
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.88fr)_minmax(0,1.12fr)]">
        <section className="panel-surface rounded-[2rem] p-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <StatusBadge>
              {t('project_editor.version_label', { count: project.latest_version_number ?? 0 })}
            </StatusBadge>
          </div>
      ...
      {activeTab === 'program' ? <ProjectProgramEditor project={project} /> : null}
      {activeTab === 'model' && hasIfcOutput ? (
