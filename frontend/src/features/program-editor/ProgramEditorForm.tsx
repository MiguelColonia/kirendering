...
export function ProgramEditorForm({ program, onChange }: ProgramEditorFormProps) {
  ...
  return (
    <section className="panel-surface rounded-[2rem] p-6">
      <div className="space-y-2">
        <h2 className="text-xl font-semibold tracking-[-0.03em]">{t('program_editor.title')}</h2>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        ...
        <label className="text-sm font-medium text-[color:var(--color-ink)]">
          ...
          <input
            ...
            onChange={(event) =>
              onChange({
                ...program,
                num_floors: Number(event.target.value),
              })
            }
            className={createInputClasses()}
          />
        ...
        <label className="text-sm font-medium text-[color:var(--color-ink)]">
          ...
          <input
            ...
            onChange={(event) =>
              onChange({
                ...program,
                floor_height_m: Number(event.target.value),
              })
            }
            className={createInputClasses()}
          />
        ...
        <label className="text-sm font-medium text-[color:var(--color-ink)]">
          ...
          <input
            ...
            onChange={(event) => updateMixCount(Number(event.target.value))}
            className={createInputClasses()}
          />
        </label>
          ...
          <input
            ...
            onChange={(event) => {
              ...
              updateTypology({
                ...
                max_useful_area: Math.max(typology.max_useful_area, nextMin + 4),
              })
            }}
            className={createInputClasses()}
          />
        ...
        <label className="text-sm font-medium text-[color:var(--color-ink)]">
          ...
          <input
            ...
            onChange={(event) => {
              ...
              updateTypology({
                max_useful_area: Math.max(nextMax, typology.min_useful_area + 4),
              })
            }}
            className={createInputClasses()}
          />
      ...
      <div className="mt-5 rounded-[1.5rem] border border-[color:var(--color-line)] bg-[color:var(--color-teal-soft)] px-5 py-4 text-sm text-[color:var(--color-teal)]">
        ...
        <p className="mt-2 text-2xl font-semibold tracking-[-0.04em]">{unitsPerFloor.toFixed(1)}</p>
      </div>
    </section>
  )
}