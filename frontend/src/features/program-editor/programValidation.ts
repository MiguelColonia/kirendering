...
function createRoomSchema(t: TFunction) {
  return z.object({
    ...
    min_area: z
      .number()
      .positive(t("program_editor.validation.positive_number")),
    min_short_side: z
      .number()
      .positive(t("program_editor.validation.positive_number")),
  });
  ...
  return z
    .object({
      ...
      num_bedrooms: z
        .number()
        .int()
        .min(0, t("program_editor.validation.non_negative_integer")),
      num_bathrooms: z
        .number()
        .int()
        .min(1, t("program_editor.validation.bathrooms_minimum")),
      rooms: z
        .array(createRoomSchema(t))
        .min(1, t("program_editor.validation.rooms_required")),
    })
  ...
  return z
    .object({
      ...
      num_floors: z
        .number()
        .int()
        .min(1, t("program_editor.validation.floors_minimum")),
      floor_height_m: z
        .number()
        .positive(t("program_editor.validation.positive_number")),
      typologies: z
        .array(typologySchema)
        .min(1, t("program_editor.validation.typologies_required")),
      mix: z
        .array(
          z.object({
            ...
            count: z
              .number()
              .int()
              .min(1, t("program_editor.validation.mix_minimum")),
          }),
        )
...
export function synchronizeMix(program: Program): Program {
  const counts = new Map(
    program.mix.map((entry) => [entry.typology_id, entry.count]),
  );
  return {
    ...program,
